const DistributionCenter = (() => {
  ...
  async function load(){
    ...
    try{
      ...
      <div class="card"><div class="name">⚡ Ofertas prioritárias</div><p class="note">Compartilhe diretamente uma oferta selecionada pelo cérebro comercial. Cada clique passa pelo rastreamento do NEXORA.</p>${products.length?`<div class="list">${products.map(p=>{const url=productUrl(p),text=productText(p);return `<div class="card"><div class="name">${esc(p.name)}</div><div class="note">Score ${Number(p.score||0)} · ${p.commissionRate!=null?`${Math.round(Number(p.commissionRate)*100)}% comissão`:''}</div><div class="actions"><button class="btn" data-native-share="${esc(url)}" data-share-title="${esc(p.name)}" data-share-text="${esc(text)}">📲 Compartilhar agora</button><a class="btn" href="${esc(whatsapp(url,text))}" target="_blank" rel="noopener noreferrer">WhatsApp</a><a class="btn secondary" href="${esc(telegram(url,text))}" target="_blank" rel="noopener noreferrer">Telegram</a><button class="btn secondary" data-copy="${esc(url)}">Copiar link</button></div></div>`}).join('')}</div>`:'<p class="note">Nenhuma oferta prioritária disponível neste momento.</p>'}</div>
      <div class="card"><div class="name">🚀 Divulgação rápida</div><p class="note">Para aumentar os primeiros cliques sem anúncios pagos, use o compartilhamento nativo ou envie a página pública para seus contatos e canais permitidos.</p><div class="actions"><button class="btn" data-native-share="${esc(offers)}">Compartilhar ofertas</button><a class="btn" href="${esc(whatsapp(offers))}" target="_blank" rel="noopener noreferrer">WhatsApp</a><a class="btn secondary" href="${esc(telegram(offers))}" target="_blank" rel="noopener noreferrer">Telegram</a><button class="btn secondary" data-copy="${esc(offers)}">Copiar ofertas</button></div><p class="note">Página pública: <a href="${esc(offers)}" target="_blank" rel="noopener noreferrer">/ofertas</a> · <a href="${esc(content)}" target="_blank" rel="noopener noreferrer">/conteudo</a></p></div>

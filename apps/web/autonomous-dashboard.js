(async()=>{
  ...
  try{
    ...
    const [dashboardResponse,statsResponse,contentResponse,brainResponse,allocationResponse,marketplaceResponse]=await Promise.all([
      ...
      fetch(base+'/api/marketplace-radar?limit=8',{signal:controller.signal,cache:'no-store'})
    ]);
    clearTimeout(timer);
    ...
    const radar=marketplaceResponse.ok?(await marketplaceResponse.json())?.radar||{}:{};

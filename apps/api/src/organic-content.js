...
function pageShell({title,description,canonical,body,schemaExtra={}}){
  const schema = JSON.stringify({
    ...
    isPartOf:{'@type':'WebSite',name:'NEXORA AI',url:String(canonical || '').split('/conteudo')[0].split('/ofertas')[0]},
    ...schemaExtra
  }).replace(/<\/script/gi,'<\\/script');
  return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)}</title><meta name="description" content="${escapeHtml(description)}"><meta name="google-site-verification" content="STpdbs0bEZfr8ODfX3O_ixNgAe_qY6XY2PUxhNfSxwM"><link rel="canonical" href="${escapeHtml(canonical)}"><link rel="alternate" type="application/rss+xml" title="NEXORA AI — Guias de compra" href="/feed.xml"><meta name="robots" content="index,follow"><meta property="og:title" content="${escapeHtml(title)}"><meta property="og:description" content="${escapeHtml(description)}"><meta property="og:type" content="website"><meta property="og:url" content="${escapeHtml(canonical)}"><script type="application/ld+json">${schema}</script><style>body{font-family:system-ui,-apple-system,Segoe UI,sans-serif;max-width:900px;margin:auto;padding:24px;line-height:1.55;color:#172033}a{color:#155eef}.card,.seo-article{border:1px solid #d9dfeb;border-radius:16px;padding:20px;margin:16px 0}.cta,.btn{display:inline-block;padding:12px 18px;border-radius:10px;background:#172033;color:#fff;text-decoration:none}.muted,.note{color:#64748b}.grid{display:grid;gap:16px}@media(min-width:700px){.grid{grid-template-columns:1fr 1fr}}</style></head><body>${body}</body></html>`;
}
...
function fallbackProductPage(product, origin){
  ...
  return pageShell({title:`${product.name} — guia e oferta`,description:`Informações para comparar ${product.name} e consultar a oferta disponível no parceiro.`,canonical,body,schemaExtra:{'@type':'Article',headline:`${product.name} — guia e oferta`,mainEntityOfPage:canonical}});
}
...
export async function handleOrganicSitemap(request, env){
  ...
  const origin = new URL(request.url).origin;
  let urls = [{loc:`${origin}/ofertas`},{loc:`${origin}/conteudo`}];
  if(env?.DB){
    const result = await env.DB.prepare("SELECT slug,updated_at FROM content_items WHERE status='published' ORDER BY created_at DESC LIMIT 500").all();
    urls.push(...(result.results || []).map((p) => ({loc:`${origin}/conteudo/${slugify(p.slug)}`,lastmod:p.updated_at})));
    ...
    urls.push(...(products.results || []).map((p) => ({loc:`${origin}/conteudo/${slugify(p.name)}`})));
  }
  const uniqueUrls = [...new Map(urls.map((item) => [item.loc,item])).values()];
  const xml = `<?xml version="1.0" encoding="UTF-8"?><urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${uniqueUrls.map((item) => `<url><loc>${escapeHtml(item.loc)}</loc>${item.lastmod?`<lastmod>${escapeHtml(item.lastmod)}</lastmod>`:''}</url>`).join('')}</urlset>`;

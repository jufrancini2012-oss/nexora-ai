...
export async function handleServiceOrganic(request) {
  ...
  if (url.pathname === '/servicos' || url.pathname === '/servicos/') {
    ...
    const body = `<header><h1>💼 NEXORA Serviços</h1><p>Serviços de revisão, conteúdo, documentos e materiais educacionais executados pela NEXORA. Envie seu pedido para receber um orçamento adequado ao escopo.</p></header><main class="grid">${cards}</main><footer class="muted"><p>Valores são referências iniciais. O orçamento final considera volume, complexidade, prazo e requisitos.</p><p><a href="/">Conhecer a NEXORA AI</a></p></footer>`;
    return new Response(shell({title:'NEXORA Serviços — revisão, conteúdo e materiais',description:'Serviços de revisão, textos profissionais, currículos, redes sociais, apresentações e materiais educacionais.',canonical:`${origin}/servicos`,body}),{headers:{'content-type':'text/html;charset=utf-8','cache-control':'public,max-age=300'}});
  }

...
export async function runAutonomyCycle(env, options = {}) {
  ...
  if (env?.DB) await env.DB.prepare(`INSERT INTO autonomy_runs (id,started_at,status,trigger) VALUES (?,?,?,?,?)`.replace('?,?,?,?,?', '?,?,?,?')).bind(runId, startedAt, 'running', trigger).run();
  ...
  try {
    ...
    if (env?.DB) await env.DB.prepare(`UPDATE autonomy_runs SET finished_at=?,status=?,researched_count=?,selected_count=?,action_count=? WHERE id=?`)
      .bind(finishedAt, 'completed', researchedCount, Number(selectedCount), 0, runId).run();
    return { ok: true, runId, trigger, status: 'completed', researchedCount, selectedCount: Number(selectedCount), growth };
  } catch (error) {
    const finishedAt = new Date().toISOString();
